...
internal static class FrontendAuthEnvironment
{
    ...
    internal const string FrontendOriginVariableName = "VITE_TENAX_AUTH_FRONTEND_ORIGIN";
    ...
    internal static IReadOnlyDictionary<string, string> GetDevelopmentViteEnvironment(Func<string, string?> getEnvironmentVariable)
    {
        ...
        var frontendOrigin = GetNonEmptyValue(getEnvironmentVariable, FrontendOriginVariableName);
        if (frontendOrigin is not null)
        {
            environment[FrontendOriginVariableName] = frontendOrigin;
        }

        var redirectUri = GetNonEmptyValue(getEnvironmentVariable, RedirectUriVariableName);
        if (redirectUri is not null)
        {
            environment[RedirectUriVariableName] = redirectUri;

            var postLogoutRedirectUri = GetNonEmptyValue(getEnvironmentVariable, PostLogoutRedirectUriVariableName) ?? redirectUri;
            environment[PostLogoutRedirectUriVariableName] = postLogoutRedirectUri;
        }
